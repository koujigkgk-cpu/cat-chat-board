...
   /* --- スマホ専用の2列レイアウト設定 (画面幅 768px 以下) --- */
@media (max-width: 768px) {
    .app-container {
        display: flex;      /* 横並びを強制 */
        flex-direction: row; /* 縦にしない */
        gap: 8px;           /* 隙間を最小限に */
        padding: 5px;       /* 外側の余白も削る */
...
        gap: 10px;
        width: 30%;         /* 画面の3割を左列に割り当て */
        flex-shrink: 0;     /* 潰れないように固定 */
    }

    /* 右列（タイムライン） */
    .main-column {
        width: 68%;         /* 画面の約7割をタイムラインに */
        flex-grow: 1;
    }

    /* スマホ用に文字サイズや画像を微調整（はみ出し防止） */
    .side-column {
        padding: 10px 5px;
        font-size: 10px;    /* 文字を小さくして収める */
    }
